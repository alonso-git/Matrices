...
class MatrixSize:
    ...
    def __init__(self, m: int = 0, n: int = 0):
        self.__m = m
        self.__n = n
    ...
    def __repr__(self) -> str:
        return f"Matrix size: {self.__m} rows x {self.__n} cols"
    
    def __str__(self) -> str:
        return f"({self.__m},{self.__n})"
...
class Matrix():
    ...
    def __add__(self, mat) -> list[list[Fraction]]:
        if isinstance(mat, Matrix):
            if mat.dim == self.dim:
                result = []
                result.append([[a + b for a,b in zip(row_A, row_B)] for row_A,row_B in zip(self, mat)])
